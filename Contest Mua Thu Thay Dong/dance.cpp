...
namespace sub_task1{
	int dp[1005][1005];
	//dp[i,s] is the minimum diff counts up to i and s is the number of groups have been made up.
	void solve(){
		sort(a+1,a+n+1);
		memset(dp, 0x3f, sizeof dp);
		fod(i,0,n) dp[i][0] = 0;
		fod(i,2,n) fod(s,1,k){
			dp[i][s] = min(dp[i-1][s], dp[i-2][s-1] + a[i] - a[i-1]);
		}
		int res = INF;
		fod(i,1,n) res = min(res, dp[i][k]);
		cout << res << el;
	}	
