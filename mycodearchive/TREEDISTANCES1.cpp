...
signed main(){
	...
	calc(1,0);
	int far1 = 0;
	fod(i,1,n) far1 = max(far1, h[i]);
	fod(i,1,n) if(far1 == h[i]){
		far1 = i;
		break;
	}
	int far2 = 0; h[far1] = 0;
	calc(far1,0);
	fod(i,1,n) far2 = max(far2, h[i]);
	fod(i,1,n) if(far2 == h[i]){
		far2 = i;
		break;
	}
	// cout << far1 << " " << far2 << el;
	...
	dfs(1,0);
	fod(i,1,n){
		cout << max(dist(i,far1), dist(i,far2)) << " ";
	}
	cerr << "\nTime: " << clock() << "ms\n";
	cerr << "⏁⊑⟒ ⋔⍜⍜⋏ ⍙⏃⌇ ⌇⍜ ⏚⟒⏃⎍⏁⟟⎎⎍⌰ ⏁⊑⏃⏁ ⏁⊑⟒⍀⟒ ⍙⏃⌇ ⏃ ⋔⟟⍀⍀⍜⍀ ⟟⋏ ⏁⊑⟒ ⍜☊⟒⏃⋏.\n";
	return 0;
