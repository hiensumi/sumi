...
int n, m, child[base], f[base], ans[base];
vector <int> adj[base];
...
void get(int u, int pu){
	ans[u] = ans[pu] - 2 * child[u] + n;
	// ans[u] = ans[pu] - 2 * child[u] + n - f[u] + f[u] 
	for(int v : adj[u]){
		if(v == pu) continue;
		get(v,u);
	}
...
signed main(){
	...
	// get(1,0);
	ans[1] = f[1];
	for(int v : adj[1]) get(v,1);
	fod(i,1,n) cout << ans[i] << " ";
	cerr << "\nTime: " << clock() << "ms\n";
	cerr << "⏁⊑⟒ ⋔⍜⍜⋏ ⍙⏃⌇ ⌇⍜ ⏚⟒⏃⎍⏁⟟⎎⎍⌰ ⏁⊑⏃⏁ ⏁⊑⟒⍀⟒ ⍙⏃⌇ ⏃ ⋔⟟⍀⍀⍜⍀ ⟟⋏ ⏁⊑⟒ ⍜☊⟒⏃⋏.\n";
	return 0;
}