...
int n, f[base][100], g[base][2];
string s;
int find(string t){
	int v = t.size();
	fok(x,v,0){
		if(t.substr(v-x,x) == s.substr(0,x)) return x;
	}
	return 0;
}
signed main(){
	...
	cin >> n >> s;
	int m = s.size();
	fod(i,0,m-1) g[i][0] = find(s.substr(0,i) + "0"), g[i][1] = find(s.substr(0,i) + "1");
	f[0][0] = 1;
	fod(i,0,n-1){
		fod(p,0,m-1){
			fod(t,0,1){
