...
int dp[19][2][2][18 * 9 + 1];
int solve(int i, int tight, int sum, string a, string b, int val){
    if(i == a.size()){
        if(sum == x){
        	return 1;
        }
    ...
    fod(j,0,limit){
        if(tight == 0 && j == a[i] - '0'){
            cnt += solve(i + 1,0, sum + j, a,val * 10 + j);
        }
        else {
            cnt += solve(i + 1,1, sum + j, a, val * 10 + j);
        }
    }
