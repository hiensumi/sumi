...
int n, m, f[base][101], a[base];
//gọi f[i][j] là số cách để dãy từ 1..i thỏa mãn với a[i] = j
signed main(){
	...
	cin >> n >> m;
	fod(i,1,n) cin >> a[i];
	if(a[1] == 0) fod(i,0,m) f[1][i] = 1;
	else f[1][a[1]] = 1;
	fod(i,2,n){
		if(a[i] != 0){
			if(a[i] >= 1 and a[i] <= m) add(f[i][a[i]],f[i-1][a[i]]);
			if(a[i] + 1 >= 1 and a[i] + 1 <= m) add(f[i][a[i]], f[i-1][a[i]+1]);
			if(a[i] - 1 >= 1 and a[i] - 1 <= m) add(f[i][a[i]], f[i-1][a[i]-1]);
		}
		else{
			fod(j,1,m){
				add(f[i][j],f[i-1][j]);
				if(j != m) add(f[i][j], f[i-1][j+1]);
				if(j != 1) add(f[i][j], f[i-1][j-1]);
			}
		}
	}
	int res = 0;
	fod(i,1,m) add(res,f[n][i]);
	cout << res;
	cerr << "\nTime: " << clock() << "ms\n";
	cerr << "⏁⊑⟒ ⋔⍜⍜⋏ ⍙⏃⌇ ⌇⍜ ⏚⟒⏃⎍⏁⟟⎎⎍⌰ ⏁⊑⏃⏁ ⏁⊑⟒⍀⟒ ⍙⏃⌇ ⏃ ⋔⟟⍀⍀⍜⍀ ⟟⋏ ⏁⊑⟒ ⍜☊⟒⏃⋏.\n";
	return 0;
