...
int dp[base],n, near[base], a[base], st[4*base];
...
signed main(){
	...
	build(1,1,n);
	// upd(1,1,n,3,3,10);
	// cout << get(1,1,n,3,3) << el;
	fod(i,2,n){
		if(near[i] != -1){
			// fod(j,near[i],i-1) dp[i] = min(dp[i], dp[j] + 1);
			int g = get(1,1,n,near[i],i-1);
			int r = get(1,1,n,i,i);
			if(r > g + 1) upd(1,1,n,i,i,g+1);
		}
	}
	int res = get(1,1,n,n,n);
