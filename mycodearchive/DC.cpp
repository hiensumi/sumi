...
int n,m,p,T,B, a[base], b[base], near[base];
signed main(){
	...
	b[p+2] = 1e18;
	fod(i,1,m){
		int j = lower_bound(b, b + p + 2, a[i]) - b;
		if(abs(b[j-1] - a[i]) <= abs(b[j] - a[i])) near[i] = b[j-1];
		else near[i] = b[j];
	}
	int res = 0;
	near[0] = 1;
	...
	fod(i,0,m){
		int ans = T * (a[i+1] - a[i]);
		ans = min(ans, abs(near[i] - a[i]) * T + B + T * abs(near[i+1] - a[i+1]));
		res += ans;
	}
	cout << res;
	kill();
