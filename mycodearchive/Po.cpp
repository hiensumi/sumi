...
//"Success isn't determined by how many times you win, but by how you p(lay the week after you lose." -Pele...
#define name ""
...
namespace sub_task1{
    void solve(){
 		...
 		int res = 0;
 		fod(i,1,n){
 			while(st.size() and st.top() > a[i]) st.pop();
 			if(st.size() and st.top() == a[i]){
 				continue;
 			} 
 			if(a[i] != 0){
 				res++;
