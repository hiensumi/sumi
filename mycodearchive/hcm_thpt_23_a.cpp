...
namespace sub_task1{
	int dd[base], pos[base];
	void solve(){
		int res = 0;
		fod(i,1,n) pos[a[i]] = i;
		BIT.init(n);
		fod(i,1,n) BIT.range_add(i,i,1);
		fod(i,1,n){
			if(odd(i)){
				int val = i/2 + 1;
				// dd[val] = 1;
				int cnt = 0;
				BIT.range_add(pos[val],pos[val],-1);
				cnt = BIT.get(1,pos[val] - 1);
				// fod(j,1,pos[val] - 1) if(dd[a[j]] == 0) cnt++;
				cout << cnt << el;
				// pos[val] = val;
			}
			else{
				int cnt = 0, val = n - i/2 + 1;
				// dd[val] = 1;
				BIT.range_add(pos[val],pos[val],-1);
				cnt = BIT.get(pos[val] + 1, n);
				// fod(j,pos[val] + 1,n) if(dd[a[j]] == 0) cnt++;
				// pos[val] = val;
				cout << cnt << el;
			}
		}
