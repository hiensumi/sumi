...
namespace solution{
 	...
 	int tour[base], st[base], en[base], dp[base];
	...
	void sub1(){
		dfs(1,0);
		BIT.init(n);
		// fod(i,1,n) cout << st[i] <<  " " << en[i] << el;
		fod(i,1,n){
			cout << BIT.get(st[a[i]], st[a[i]]) << el;
			// fod(v,st[a[i]], en[a[i]]) dp[tour[v]]++;
			BIT.range_add(st[a[i]], en[a[i]],1);
		}
	}
	void solve(){
		inp();
