...
int n, a[base], s, dd[base], f[base], t[4*base];
mset <int> ms;
signed main(){
	...
	fod(i,1,n) f[i] = f[i-1] + dd[i];
	// l..r thỏa khi f[r] - f[l-1] > 0
	// f ko tăng dần
	// với f[r] tìm những thằng 1 .. l-1 mà < f[r]
	int res = 0;
	// fod(i,1,n){
	// 	fod(j,1,i) if(f[i] - f[j-1] > 0) res++; 
	// }
	ms.insert(0);
	fod(i,1,n){
		int d = ms.order_of_key(f[i]);
		res += d;
		ms.insert(f[i]);
	}
	cout << res;
