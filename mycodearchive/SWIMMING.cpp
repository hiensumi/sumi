...
Top merge(Top A, Top B){
	...
	int i = 0, j = 0;
	fod(t,0,1){
		tmp.val[t] = A.val[i] > B.val[j] ? A.val[i++] : B.val[j++];
	}
	return tmp;
}
...
Top get(int i, int l, int r, int u, int v){
	if(v < l or r < u) return Top();
	if(u <= l and r <= v) return st[i];
	int mid = l + r >> 1;
	return merge(get(2*i, l, mid, u, v), get(2*i+1, mid+1, r, u, v));
}
signed main(){
	...
	int res = 0;
	fod(i,1,n){
		// j - i + 1 = 2 * k + 2 -> j = 2 * k + 2 - 1 + i = 2 * k + 1 + i
		int j = 2 * k + 1 + i;
		if(j > n) break;
		Top x = get(1,1,n,i,j);
		// if(x.val[0] == -0x3f or x.val[1] == -0x)
		res = max(res, x.val[0] +  x.val[1]);
	}
	cout << res;
	cerr << "\nTime: " << clock() << "ms\n";
	cerr << "⏁⊑⟒ ⋔⍜⍜⋏ ⍙⏃⌇ ⌇⍜ ⏚⟒⏃⎍⏁⟟⎎⎍⌰ ⏁⊑⏃⏁ ⏁⊑⟒⍀⟒ ⍙⏃⌇ ⏃ ⋔⟟⍀⍀⍜⍀ ⟟⋏ ⏁⊑⟒ ⍜☊⟒⏃⋏.\n";
