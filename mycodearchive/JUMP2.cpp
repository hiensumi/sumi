...
int n,m,k, a[base], b[base], down[base][base], righ[base][base], f[base][base];
signed main(){
	...
	fod(i,1,n) cin >> a[i];
	fod(i,1,m) cin >> b[i];
	f[n][m] = down[n][m] = righ[n][m] = 1;
	fok(i,n,1) fok(j,m,1){
		int val = ((a[i] + b[j]) % k) + 1;
		add(f[i][j], righ[i][j+1] - righ[i][min(j + 1 + val, m + 1)]);
		add(f[i][j], down[i+1][j] - down[min(i + 1 + val, n + 1)][j]);
		righ[i][j] = down[i][j] = 0;
		add(righ[i][j], righ[i][j+1] + f[i][j]);
		add(down[i][j], down[i+1][j] + f[i][j]);
	}
	cout << f[1][1];
	kill();
}