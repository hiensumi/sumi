...
namespace sub_task3{
    void solve(){
        ...
        if(k >= n) res = (mul1 % MOD * mul2 % MOD + MOD) % MOD;

        int left = k % n, x = k / n;
        res = (res % MOD * max(1ll, x) % MOD) % MOD;
        fod(i,n+1,2*n) b[i] = b[i - n];
        fod(i,1,2*n) b[i] += b[i-1];

        fod(i,1,n) (res += a[i] % MOD * (b[i + left - 1] - b[i-1]) % MOD) %= MOD;
