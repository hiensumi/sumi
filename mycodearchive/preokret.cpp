...
int n, hn[base], city = 0, oppo = 0, draw = 1;
void inp(){
	cin >> n;
	fod(i,1,n){
		cin >> hn[i];
...
namespace sub_task1{
    void solve(){
    	...
    	fod(i,1,n){
    		if(hn[i] == 1) city++; else oppo++;
    		if(city < oppo){
    			cur = 0;
    			fod(j,i+1,n) if(hn[j] == 1) cur++; else break;
    			if(city + cur > oppo)maxi(res, cur);
    		}
    		else if(oppo < city){
    			cur = 0;
    			fod(j,i+1,n) if(hn[j] == 2) cur++; else break;
    			if(oppo + cur > city) maxi(res, cur);
    		}
    	}
    	cout << max(res, 1ll);
    }	
