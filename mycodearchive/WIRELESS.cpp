...
int n, q, p[base][21], f[base], h[base], node[base];
vector <ii> adj[base];
...
signed main(){
	...
	fod(aowiduwa,1,q){
		int k; cin >> k;
		fod(i,1,k) cin >> node[i];
		vector<int> dad;
		fod(i,1,k) fod(j,i+1,k){
			dad.pb(lca(node[i],node[j]));
		}
		int res = 1e18;
		for(int fn : dad){
			int ans = 0;
			fod(i,1,k) ans += dist(fn,node[i]);
			res = min(res, ans);
		}
		cout << res << el;
	}
