...
int calc(int n, int k){
	...
	if(k <= n){
		int time = k + 1 >> 1;
		int val = time * (time + 1)/2;
		if(val > 0 and time > 0) res = k * time - 2 * val + time;
	}
    else{
    	int time = min(k - n, n + 1);
    	int val = time * (time + 1) >> 1;
    	res += n * time - val + time;
    	time = (k + 1)/2 - (k - n + 1) + 1;
    	val = time * (((k+1)/2) + k - n + 1)/2;
    	if(time > 0 and val > 0)res += k * time - 2 * val + time;
    }
    return res;
}
signed main(){
