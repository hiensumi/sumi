...
int n, m, h,w, a[base][base], f[base][base], b[base][base];
bool nt(int x, int y){
	return 1 <= x and x <= n and 1 <= y and y <= m;
}
int calc(int i, int j, int u, int v){
	return f[u][v] - f[i-1][v] - f[u][j-1] + f[i-1][j-1];
}
bool check(int x){
	fod(j,0,m) f[0][j] = 0;
	fod(i,0,n) f[i][0] = 0;
	fod(k1,1,n) fod(k2,1,m){
			f[k1][k2] = f[k1-1][k2] + f[k1][k2-1] - f[k1-1][k2-1] + (a[k1][k2] <= x);
		}
	fod(i,1,n) fod(j,1,m){
		// memset(f,0,sizeof f);
		int u = i + h - 1, v = j + w - 1;
		if(!nt(u,v)) continue;
		if(calc(i,j,u,v) >= (w * h + 1)/2) return 1; 
	}
	return 0;
}
signed main(){
