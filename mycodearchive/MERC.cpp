...
int n, m, par[base], h[base], p[base][21];
set <int> adj[base], g[base];
int find(int u) {
    if (par[u] == u) return u;
...
int lca(int u, int v){
	if(h[u] < h[v]) swap(u,v);
	int k = h[u] - h[v];
	for(int j = 0; mask(j) <= k; j++) if(k & mask(j)) u = p[u][j];
	if(u == v) return u;
	int d = log2(h[u]);
...
int low[base], num[base], stt = 0, res = 0, tp[base], vao[base], dd[base];
stack <int> s;
void DFS(int u, int p){
	stt++;
	num[u]=stt,low[u]=stt;
	s.push(u);
	for(int v:g[u]) if(dd[v]==0 and v != p){
