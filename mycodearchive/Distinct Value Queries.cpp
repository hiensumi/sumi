...
int n, q, a[base], id[base];
// id[i] là vị trí tiếp theo mà a[i] xuất hiện
map <int,int> mp;
...
int get(int i, int l, int r, int u, int v, int u){
	if(r < u or v < l) return 0;
	if(u <= l and r <= v) return st[i].size() - (upper_bound(st[i].begin(), st[i].end(), x) - st[i].begin());;
	int mid = l + r >> 1;
