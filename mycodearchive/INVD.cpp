...
signed main(){
	...
	cin >> n >> d;
	fod(i,1,n) cin >> a[i];
	// i < j < k: a[i] >= a[j] + d, a[j] >= a[k] + d
	// a[k] + d <= a[j] <= a[i] - d
	fod(i,1,n) g.insert(a[i]);
	int res = 0;
	fod(j,1,n){
		g.erase(g.lower_bound(a[j]));
		f.insert(a[j]);
		// cout << "f: ";
		// for(int x : f) cout << x << " ";
		// cout << "g: ";
		// for(int x : g) cout << x << " "; 
		// cout << el;
		auto x = g.lower_bound(a[j] - d);
		int d1 = distance(g.begin(), x) + 1;
		if(x == g.end()) continue;
		auto y = f.lower_bound(a[j] + d);
		if(y == f.end()) continue;
		int d2 = distance(y, f.find(*f.rbegin())) + 1;
		res += d1 * d2;
	}
	cout << res;
	kill();
