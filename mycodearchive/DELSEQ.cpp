...
namespace sub_task1{
    int dp[1001][2001], dd[1001][2001];
	int calc(int i, int s){
        if(i == 0) return 0;
        if(dd[i][s]) return dp[i][s];
        dd[i][s] = 1;
        int best = max(0ll, calc(i - 1, s) + a[i]);
        fod(j,0,k){
            if((s & mask(j)) > 0 and mask(j) <= i){
                int p = s xor mask(j);
                maxi(best, calc(i - mask(j), p));
            }
        }
        return dp[i][s] = best;
    }      
