...
// dp[i][j] là nghiệm tới i và là ptử thứ j trong tập K
int n, k, dp[base][52];
void inp(){
	cin >> n >> k;
}
...
namespace sub_task1{
	vector <int> divs[base + 5];
	void solve(){
		fod(i,1,n){
			for(int j = i; j <= base; j += i) divs[j].pb(i);
		}
		fod(i,1,n) dp[i][1] = 1;
		fod(i,1,n) fod(s,2,k){
			for(int j : divs[i]) add(dp[i][s], dp[j][s-1]);
		}
		int res = 0;
		fod(i,1,n) add(res, dp[i][k]);
		cout << res;
	}	
