...
namespace sub_task1{
	// dp i j là số điểm bị trừ ít nhất
	// khi xét tới i và còn j lần đổi mức
	// năng lượng
	int dp[405][405];
	void solve(){
		...
		fod(i,1,n) fod(j,1,K+1){
			int ma = -INF, ans = INF;
			fok(k,i,1){
				ma = max(ma, a[k]);
				ans = min(ans, dp[k-1][j-1] + ma * (i - k + 1) - (pre[i] - pre[k-1]));
			}
			dp[i][j] = ans;
		}
		cout << dp[n][K+1];
