...
#define name ""
namespace solution{
    int n, a[base], suma = 0, sumb = 0, b[base];
	void inp(){
		suma = 0;
		sumb = 0;
		cin >> n;
		fod(i,1,n) cin >> a[i], suma += a[i];
		fod(i,1,n) cin >> b[i], sumb += b[i];
	}
	int knapsack(int weight){
		// a takes as weight
		// b takes as value
		vector <vector <int>> dp(n + 1, vector <int>(weight + 1));
		fod(i,1,n) fod(j,0,weight){
			dp[i][j] = dp[i-1][j];
	...
	bool check(int mid){
		int val = knapsack(suma - mid) >= mid;
		return val;
	}
	void sub1(){
		int l = 0, r = min(suma,sumb), ans = 0;
		while(l <= r){
			int mid = l + r >> 1;
