...
namespace sub_task1{
	void solve(){
		...
		while(ms.size() > 1){
			int mi = *ms.begin(), ma = *ms.rbegin();
			if(ms.find(mi) != ms.end()) ms.erase(ms.find(mi));
			if(ms.find(ma) != ms.end()) ms.erase(ms.find(ma));
			ms.insert(mi | ma);
		}
		cout << *ms.begin() << el;
	}	
...
namespace sub_task2{
	void solve(){
		vector <int> bit1(31 + 1);
		fod(i,1,n){
			fod(j,0,31) if(BITpos(a[i], j)) bit1[j]++;
		}	
		int res = 0;
		fod(j,0,31) if(bit1[j] > 1) res += mask(j);
		cout << res << el;
	}
}
