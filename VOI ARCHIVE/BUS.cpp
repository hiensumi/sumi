...
int n, m, s, t, lab[base];
struct DL{ int type, u, v, c ;  
	bool operator< (const DL& other){
		return c < other.c;
	}};
struct DT { int v, c, type;};
ve < ve<DT> > g;
ve <DL> e;
bool ok1 = 1;
	...
	int dis[2][base];
   	void dji(int *dis, int s){
   		...
   		while(!pq.empty()){
   			...
   			for(auto [v,c,type] : g[u]){
   				// cout << v << el;
   				if(mini(dis[v], max(val, c))){
   					pq.push(mp(-dis[v], v));
   				}
   			}
