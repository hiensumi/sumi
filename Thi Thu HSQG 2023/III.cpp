...
namespace sub_task1{
	bool check(int x, int y){
		int val = bp(x * x + x * y, 2) + bp(y * y + x * y, 2);
		return val > 8;
	}
	void solve(){
		int tt = mask(m) - 1;
		...
		fod(x,0,tt){
			...
			fod(i,1,n){
				fod(j,1,n) dd[j] = 0;
				dfs(i);
				fod(j,1,n) if(i != j and check(i,j) and dd[j] == 1){
					cnt++;
				}
			}
			res.pb(ii(cnt,x));
		...
		sort(all(res));
		// for(ii x : res) cout << x.fi << el;
		ii ans = res.back();
		cout << ans.fi << " ";
		fod(i,1,m){
			if(BITpos(ans.se, i - 1)) cout << 'F';
			else cout << 'B';
		}
		cout << el;
...
signed main(){
	...
	int sub, Test; cin >> sub >> Test;
	init();
	while(Test--){
		inp();
		if(m <= 16) sub_task1 :: solve();
